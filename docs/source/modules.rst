...
parameters, concatenated with ``fn_kwargs`` and passed to ``fn`` for
evaluation. These parameters are updated during fitting.

Optional \* ``plot_fns`` \* ``plot_fn_idx`` \* ``priors``

The only *mandatory* field each module dict is ``fn``. All other fields
