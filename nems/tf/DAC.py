...
def make_three_trials(all_trials, all_epochs, all_specs, stim_dict, cut_dir, fs, cats,ee,
    ...
    trial = np.zeros(trial_length * fs)
    bin_length = int(trial_length / number_bins * fs)
    bin_start_times = [bin_length * i for i in range(number_bins)]
    trial_labels = ['A1','A2','A3','A4','A5','A6']

    all_trials, all_epochs, all_specs = generate_triad(all_trials, all_epochs, all_specs, trial, trial_labels,
                                            bin_start_times, cats, fs, seg_dict, seg_label_list,ee)

    return all_trials, all_epochs, all_specs, trial_labels
...
def organize_sounds(start_times, cats, fs, seg_dict, seg_label_list):
    ...
    start_times_s = [st / fs for st in start_times]
...
def populate_triad(trial, trial_labels, start_times, fs, sound_dict, sort_df, real_epochs,ee):
    bin_length = (start_times[1] - start_times[0]) / fs
    for num, pop in enumerate(trial_labels):
        ...
        row = {'name': sort_df.loc[pop]['Name'],
           'start': sort_df.loc[pop]['On'] + (ee*15),
           'end': sort_df.loc[pop]['Off'] + (ee*15)}
        row1 = {'name': sort_df.loc[pop]['Class'],
           'start': sort_df.loc[pop]['On'] + (ee*15),
           'end': sort_df.loc[pop]['Off'] + (ee*15)}
        real_epochs = real_epochs.append(row, ignore_index=True)
        real_epochs = real_epochs.append(row1, ignore_index=True)
    row_ref = {'name': 'REFERENCE', 'start':start_times[0] + (ee*15) , 'end': 15 + (ee*15)}
    real_epochs = real_epochs.append(row_ref,ignore_index=True)
...
def sound_assigner(trial_letter, count, start, cats, seg_label_list,
    ...
    row = {'Label': f'{trial_letter}{count + 1}', 'Name': 'STIM_' + add_label, 'Class': f"CLASS_{cat}", 'On': np.round(jitter + start, 2),
...
def generate_triad(all_trials, all_epochs, all_specs, trial, trial_labels, bin_start_times,
                   cats, fs, seg_dict, seg_label_list,ee):
    ...
    single_trial, single_epoch = populate_triad(trial, trial_labels, bin_start_times, fs, sound_dict, sort_df, real_epochs,ee)
    ...
    return all_trials, all_epochs, all_specs
...
for cc in class_labels:
    ...
    sound_sets[cc] = glob.glob(f"/auto/users/hamersky/Classifier/{cc}/*.wav")
...
stim = signal.TiledSignal(data=all_specs, epochs=epochs, fs=int(fs), name='stim', recording="NAT")
stim = stim.rasterize()
