...
def make_three_trials(all_trials, all_epochs, all_specs, stim_dict, cut_dir, fs, cats,
...
def populate_triad(trial, trial_labels, start_times, fs, sound_dict, sort_df, real_epochs):
    bin_length = (start_times[1] - start_times[0]) / fs
...
def generate_triad(all_trials, all_epochs, all_specs, trial, trial_labels, bin_start_times,
    ...
    for trl in range(len(trial)):
        single_trial, single_epoch = populate_triad(trial[trl], trial_labels[trl], bin_start_times, fs, sound_dict, sort_df, real_epochs)
